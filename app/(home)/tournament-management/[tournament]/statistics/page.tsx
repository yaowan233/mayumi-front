...
const RefreshIcon = () => (<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>);
const InfoIcon = () => (<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>);
...
export default function EditStatisticsPage(props: { params: Promise<{ tournament: string }> }) {
    ...
    if (isLoading) {
        return <div className="w-full h-[50vh] flex justify-center items-center"><Spinner size="lg" color="primary" /></div>;
    }
    ...
    return (
        <div className="w-full max-w-7xl mx-auto px-4 py-8 flex flex-col gap-8 animate-appearance-in">
            ...
            {roundInfo.length > 0 ? (
                <div className="flex flex-col gap-6">
                    ...
                    <Card className="border border-default-200 dark:border-white/5 bg-content1 dark:bg-zinc-900 shadow-sm">
                        ...
                        <CardBody className="px-6 py-4 gap-6">
                            {/* 提示信息 */}
                            <div className="flex items-start gap-3 p-4 rounded-xl bg-default-100 dark:bg-white/5 text-default-600 text-sm leading-relaxed border border-default-200 dark:border-white/5">
                                <div className="text-lg mt-0.5 text-primary"><InfoIcon /></div>
                                <div>
                                    <p className="font-bold mb-1">操作说明：</p>
                                    <ul className="list-disc list-inside space-y-1 opacity-80">
                                        <li>请确保 <span className="font-mono text-primary">赛程管理</span> 中该轮次的所有 Match Link (MP 链接) 已填写正确。</li>
                ...
                // 空状态
                <div className="flex flex-col items-center justify-center py-20 border-2 border-dashed border-default-200 dark:border-white/10 rounded-xl">
                    <p className="text-default-500">暂无轮次信息，请先前往“轮次管理”添加。</p>
                    <Button as={Link} href={`/tournament-management/${tournament_name}/round`} color="primary" className="mt-4" variant="flat">
                        去添加轮次
                    </Button>
                </div>
            )}
