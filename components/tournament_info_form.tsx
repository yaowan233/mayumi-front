        ...
        <div className="flex flex-col gap-6">
            <FormSection title="基本信息" icon={<InfoIcon/>}>
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <RequiredTextField
                        label="比赛开始时间（当地时间）"
                        type="datetime-local"
                        value={utcDateTimeToLocalInput(formData.start_date)}
                        onChange={(value) => setFormData({...formData, start_date: localDateTimeInputToUtc(value) ?? ""})}
                    />
                    <RequiredTextField
                        label="比赛结束日期"
                        type="date"
                ...
                <TextField isInvalid={Boolean(registrationTimeError(formData))}>
                    <Label>选手报名开始时间（当地时间）</Label>
                    <Input
                        type="datetime-local"
                        fullWidth
                        variant="secondary"
                        value={utcDateTimeToLocalInput(formData.registration_start_time)}
                        onChange={event => setFormData({...formData, registration_start_time: localDateTimeInputToUtc(event.target.value)})}
                    />
                    <Description>仅控制选手报名，Staff 招募默认开放。留空表示选手立即开放报名；比赛开始时间作为报名截止时间。时间按浏览器所在时区填写。</Description>
                    {registrationTimeError(formData) && <FieldError>{registrationTimeError(formData)}</FieldError>}
                </TextField>
            ...
            <FormSection title="详情与规则" icon={<RuleIcon/>}>
                ...
                <MarkdownEditor
                    label="详细规则"
                    isRequired
                    placeholder="输入比赛规则，选中文字后可设置标题、加粗或插入链接…"
